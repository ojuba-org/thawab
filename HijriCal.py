...
class HijriCal:
   ...
   def fill_month_days(self):
	 ...
	 self.__g_md[row][col]=(gd,gm); gd+=1;
	 if (gd>gn):
            gd=1; gm+=1
	    if gm>12: gm=1; gy+=1
	 col+=self.__direct;
   ...
   def validate(self):
      ...
      f=0
      if self.M<1: self.M=12; self.Y-=1; f=1
      if self.M>12: self.M=1; self.Y+=1; f=1
      if self.Y<1: self.Y=1; f=1
      if self.D<1: self.D=1; f=1
      d=hijri_month_days(self.Y, self.M)
      if self.D>d : self.D=d; f=1
      return f
